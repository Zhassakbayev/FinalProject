...
<div class="view">
    <p>
        <label><fmt:message key="key.faculty_name" /> </label>
        <c:out value="${language eq 'ru' ? name_ru : name_eng}"></c:out>
    </p>

    <p>
        <label><fmt:message key="key.total_seats" /></label>
        <c:out value="${total_seats}"></c:out>
    </p>
    <p>
        <label><fmt:message key="key.budget_seats" /></label>
        <c:out value="${budget_seats}"></c:out>
    </p>

    <c:if test="${empty facultySubjects}">
        <fmt:message key="key.no_subjects_msg" />
    </c:if>
    ...
    <c:if test="${not empty facultySubjects}">
        <ol>
            <c:forEach var="subject" items="${facultySubjects}">
                <li><a
                        href="<c:url value="controller?service=viewSubject"> <c:param name="name_eng" value="${subject.nameEng}"/></c:url>">
